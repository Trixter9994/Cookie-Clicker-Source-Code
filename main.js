...
Game.Launch=function()
{
	...
	Game.Init=function()
	{
		...
		*/
		...
		Game.CheckModData=function()
		{
			...
			for (var i in Game.modSaveData)
			{
				str+='<div style="border-bottom:1px dashed rgba(255,255,255,0.2);clear:both;overflow:hidden;padding:4px 0px;">';
					...
					str+='</div>';
					str+='<div style="float:right;width:49%;text-align:right;overflow:hidden;">'+Game.modSaveData[i].length+' chars <a class="option warning" style="padding:0px 2px;font-size:10px;margin:0px;vertical-align:top;" '+Game.clickStr+'="Game.deleteModData(\''+i+'\');PlaySound(\'cookieclicker/snd/tick.mp3\');Game.ClosePrompt();Game.CheckModData();">X</a>';
					str+='</div>';
				str+='</div>';
				modsN++;
			}
			if (modsN==0) str+='No mod data present.';
			else str+='<div><a class="option warning" style="font-size:11px;margin-top:4px;" '+Game.clickStr+'="Game.deleteAllModData();PlaySound(\'cookieclicker/snd/tick.mp3\');Game.ClosePrompt();Game.CheckModData();">Delete all</a></div>';
		...
		{
			PlaySound('cookieclicker/snd/tick.mp3');
			Game.tooltip.hide();
		...
		{
			if (Game.AscendTimer==1) PlaySound('cookieclicker/snd/charging.mp3');
			if (Game.AscendTimer==Math.floor(Game.AscendBreakpoint)) PlaySound('cookieclicker/snd/thud.mp3');
			Game.AscendTimer++;
			if (Game.AscendTimer>Game.AscendDuration)//end animation and launch ascend screen
			{
				PlaySound('cookieclicker/snd/cymbalrev.mp3',0.5);
				PlaySound('cookieclicker/snd/choir.mp3');
		...
		Game.shimmerTypes={
			...
			'golden':{
				...
				{
					if (!this.spawned && Game.chimeType==1 && Game.ascensionMode!=1) PlaySound('cookieclicker/snd/chime.mp3');
					
					//set image
					var bgPic='cookieclicker/img/goldcookie.png';
					...
					{
						me.wrath=1;
						if (Game.season=='halloween') bgPic='cookieclicker/img/spookycookie.png';
						else bgPic='cookieclicker/img/wrathcookie.png';
					}
					else
					{
						me.wrath=0;
					...
					{
						bgPic='cookieclicker/img/hearts.png';
						picX=Math.floor(Math.random()*8);
					}
					else if (Game.season=='fools')
					{
						bgPic='cookieclicker/img/contract.png';
						if (me.wrath) bgPic='cookieclicker/img/wrathcontract.png';
					}
					else if (Game.season=='easter')
					{
						bgPic='cookieclicker/img/bunnies.png';
				...
				popFunc:function(me)
				{
					...
					else PlaySound('cookieclicker/snd/shimmerclick.mp3');
					me.die();
				},
			...
			'reindeer':{
				...
				initFunc:function(me)
				{
					if (!this.spawned && Game.chimeType==1 && Game.ascensionMode!=1) PlaySound('cookieclicker/snd/jingle.mp3');
				...
				popFunc:function(me)
				{
					...
					PlaySound('cookieclicker/snd/jingleclick.mp3');
					me.die();
				},
				missFunc:function(me)
				{
		...
		Game.UpdateNotes=function()
		{
			...
			for (var i in Game.Notes)
			{
				if (i<5)
				{
					...
					str='<div id="note-'+me.id+'" class="framed note '+(me.pic!=''?'haspic':'nopic')+' '+(me.desc!=''?'hasdesc':'nodesc')+'"><div class="close" onclick="PlaySound(\'cookieclicker/snd/tick.mp3\');Game.CloseNote('+me.id+');">x</div>'+pic+'<div class="text"><h3>'+me.title+'</h3>'+(me.desc!=''?'<div class="line"></div><h5>'+me.desc+'</h5>':'')+'</div></div>'+str;
					remaining--;
				}
			}
			...
			{
				str+='<div class="framed close sidenote" onclick="PlaySound(\'cookieclicker/snd/tick.mp3\');Game.CloseNotes();">x</div>';
			}
		...
		Game.Prompt=function(content,options,updateFunc,style)
		{
			...
			for (var i in options)
			{
				...
				else
				{
					...
					opts+='<a id="promptOption'+i+'" class="option" '+Game.clickStr+'="PlaySound(\'cookieclicker/snd/tick.mp3\');'+options[i][1]+'">'+options[i][0]+'</a>';
				}
			}
		...
		Game.WriteButton=function(prefName,button,on,off,callback,invert)
		{
			...
			callback+='PlaySound(\'cookieclicker/snd/tick.mp3\');';
		...
		Game.ShowMenu=function(what)
		{
			...
			if (Game.onMenu=='') PlaySound('cookieclicker/snd/clickoff.mp3');
			else PlaySound('cookieclicker/snd/clickon.mp3');
		...
		Game.ClickTinyCookie=function(){if (!Game.HasAchiev('Tiny cookie')){PlaySound('cookieclicker/snd/tick.mp3');Game.Win('Tiny cookie');}}
		...
		Game.UpdateMenu=function()
		{
			...
			if (Game.onMenu=='prefs')
			{
				...
				'<div class="listing"><a class="option" '+Game.clickStr+'="Game.toSave=true;PlaySound(\'cookieclicker/snd/tick.mp3\');">Save</a><label>Save manually (the game autosaves every 60 seconds; shortcut : ctrl+S)</label></div>'+
				'<div class="listing"><a class="option" '+Game.clickStr+'="Game.ExportSave();PlaySound(\'cookieclicker/snd/tick.mp3\');">Export save</a><a class="option" '+Game.clickStr+'="Game.ImportSave();PlaySound(\'cookieclicker/snd/tick.mp3\');">Import save</a><label>You can use this to backup your save or to transfer it to another computer (shortcut for import : ctrl+O)</label></div>'+
				'<div class="listing"><a class="option" '+Game.clickStr+'="Game.FileSave();PlaySound(\'cookieclicker/snd/tick.mp3\');">Save to file</a><a class="option" style="position:relative;"><input id="FileLoadInput" type="file" style="cursor:pointer;opacity:0;position:absolute;left:0px;top:0px;width:100%;height:100%;" onchange="Game.FileLoad(event);" '+Game.clickStr+'="PlaySound(\'cookieclicker/snd/tick.mp3\');"/>Load from file</a><label>Use this to keep backups on your computer</label></div>'+
				
				'<div class="listing"><a class="option warning" '+Game.clickStr+'="Game.HardReset();PlaySound(\'cookieclicker/snd/tick.mp3\');">Wipe save</a><label>Delete all your progress, including your achievements</label></div>'+
				...
				'<div class="listing"><a class="option" '+Game.clickStr+'="Game.CheckModData();PlaySound(\'cookieclicker/snd/tick.mp3\');">Check mod data</a><label>(view and delete save data created by mods)</label></div>'+
			...
			else if (Game.onMenu=='log')
			{
				...
				if (!Game.HasAchiev('Olden days')) str+='<div style="text-align:right;width:100%;"><div '+Game.clickStr+'="Game.SparkleAt(Game.mouseX,Game.mouseY);PlaySound(\'cookieclicker/snd/tick.mp3\');PlaySound(\'cookieclicker/cookieclicker/snd/shimmerclick.mp3');Game.Win(\'Olden days\');Game.UpdateMenu();" class="icon" style="display:inline-block;transform:scale(0.5);cursor:pointer;width:48px;height:48px;background-position:'+(-12*48)+'px '+(-3*48)+'px;"></div></div>';
			}
		...
		AddEvent(l('legacyButton'),'click',function(){PlaySound('cookieclicker/snd/tick.mp3');Game.Ascend();});
		...
		AddEvent(Game.tickerL,'click',function(event){
			...
			{
				PlaySound('cookieclicker/snd/fortune.mp3',1);
				Game.SparkleAt(Game.mouseX,Game.mouseY);
		...
		Game.Object=function(name,commonName,desc,icon,iconColumn,art,price,cps,buyFunction)
		{
			...
			/*this.levelUp=function()
			{
				...
				PlaySound('cookieclicker/snd/upgrade.mp3',0.6);
				Game.LoadMinigames();
				me.refresh();
			...
			if (this.id==0)
			{
				var str='<div class="productButtons">';
					...
					str+='<div id="productMinigameButton'+this.id+'" class="productButton productMinigameButton lumpsOnly" onclick="Game.ObjectsById['+this.id+'].switchMinigame(-1);PlaySound(Game.ObjectsById['+this.id+'].onMinigame?\'cookieclicker/cookieclicker/snd/clickon.mp3':\'cookieclicker/cookieclicker/snd/clickoff.mp3');"></div>';
				str+='</div>';
			...
			else
			{
				...
				str+='<div class="productButtons">';
					str+='<div id="productLevel'+this.id+'" class="productButton productLevel lumpsOnly" onclick="Game.ObjectsById['+this.id+'].levelUp()" '+Game.getDynamicTooltip('Game.ObjectsById['+this.id+'].levelTooltip','this')+'></div>';
					str+='<div id="productMinigameButton'+this.id+'" class="productButton productMinigameButton lumpsOnly" onclick="Game.ObjectsById['+this.id+'].switchMinigame(-1);PlaySound(Game.ObjectsById['+this.id+'].onMinigame?\'cookieclicker/cookieclicker/snd/clickon.mp3':\'cookieclicker/cookieclicker/snd/clickoff.mp3');"></div>';
					str+='<div class="productButton productMute" '+Game.getTooltip('<div style="width:150px;text-align:center;font-size:11px;"><b>Mute</b><br>(Minimize this building)</div>','this')+' onclick="Game.ObjectsById['+this.id+'].mute(1);PlaySound(Game.ObjectsById['+this.id+'].muted?\'cookieclicker/cookieclicker/snd/clickoff.mp3':\'cookieclicker/cookieclicker/snd/clickon.mp3');" id="productMute'+this.id+'">Mute</div>';
				str+='</div>';
		...
		Game.storeBulkButton=function(id)
		{
			...
			Game.storeToRefresh=1;
			if (id!=-1) PlaySound('cookieclicker/snd/tick.mp3');
		}
		...
		for (var i in Game.Objects)
		{
			...
			if (me.id>0)
			{
				...
				muteStr+='<div class="tinyProductIcon" id="mutedProduct'+me.id+'" style="display:none;background-position:-'+icon[0]+'px -'+icon[1]+'px;" '+Game.clickStr+'="Game.ObjectsById['+me.id+'].mute(0);PlaySound(Game.ObjectsById['+me.id+'].muted?\'cookieclicker/cookieclicker/snd/clickoff.mp3':\'cookieclicker/cookieclicker/snd/clickon.mp3');" '+Game.getDynamicTooltip('Game.mutedBuildingTooltip('+me.id+')','this')+'></div>';
				//muteStr+='<div class="tinyProductIcon" id="mutedProduct'+me.id+'" style="display:none;background-position:-'+icon[0]+'px -'+icon[1]+'px;" '+Game.clickStr+'="Game.ObjectsById['+me.id+'].mute(0);PlaySound(Game.ObjectsById['+me.id+'].muted?\'cookieclicker/cookieclicker/snd/clickoff.mp3':\'cookieclicker/cookieclicker/snd/clickon.mp3');" '+Game.getTooltip('<div style="width:150px;text-align:center;font-size:11px;"><b>Unmute '+me.plural+'</b><br>(Display this building)</div>')+'></div>';
			...
			if (!cancelPurchase)
			{
				...
				else
				{
					...
					if (Game.heavenlyChips>=price && !this.bought)
					{
						...
						Game.BuildAscendTree();
						PlaySound('snd/buy'+choose([1,2,3,4])+'.mp3',0.75);
						PlaySound('cookieclicker/snd/shimmerclick.mp3');
						//PlaySound('cookieclicker/snd/buyheavenly.mp3');
						success=1;
					}
				}
			}
		...
		Game.Upgrade.prototype.toggle=function()//cheating only
		{
			if (!this.bought)
			{
				...
				if (Game.CountsAsUpgradeOwned(this.pool)) Game.UpgradesOwned++;
				PlaySound('snd/buy'+choose([1,2,3,4])+'.mp3',0.75);
				if (this.pool=='prestige' || this.pool=='debug') PlaySound('cookieclicker/snd/shimmerclick.mp3');
			}
			else
			{
				this.bought=0;
		...
		Game.AssignPermanentSlot=function(slot)
		{
			...
			{
				var me=list[i];
				upgrades+=Game.crate(me,'','PlaySound(\'cookieclicker/snd/tick.mp3\');Game.PutUpgradeInPermanentSlot('+me.id+','+slot+');','upgradeForPermanent'+me.id);
			}
		...
		Game.loseShimmeringVeil=function(context)
		{
			...
			Game.upgradesToRebuild=1;
			Game.recalculateGains=1;
			PlaySound('cookieclicker/snd/spellfail.mp3',0.75);
		}
		...
		order=11000;
		new Game.Achievement('Here you go','Click this achievement\'s slot.<q>All you had to do was ask.</q>',[1,7]);Game.last.clickFunction=function(){if (!Game.HasAchiev('Here you go')){PlaySound('cookieclicker/snd/tick.mp3');Game.Win('Here you go');}};
		...
		*/
			...
			for (var i in Game.wrinklers)
			{
				...
				if (me.phase>0)
				{
					...
					if (me.selected && onWrinkler==0 && Game.CanClick)
					{
						...
						{
							if (Game.keys[17] && Game.sesame) {me.type=!me.type;PlaySound('cookieclicker/snd/shimmerclick.mp3');}//ctrl-click on a wrinkler in god mode to toggle its shininess
							else
							{
			...
			if (Game.LeftBackground)
			{
				...
				for (var i=0;i<Game.specialTabs.length;i++)
				{
					...
					if (Math.abs(Game.mouseX-x)<=24*s && Math.abs(Game.mouseY-y)<=24*s)
					{
						...
						if (Game.Click && Game.lastClickedEl==l('backgroundLeftCanvas'))
						{
							if (Game.specialTab!=Game.specialTabs[i]) {Game.specialTab=Game.specialTabs[i];Game.ToggleSpecialMenu(1);PlaySound('cookieclicker/snd/press.mp3');}
							else {Game.ToggleSpecialMenu(0);PlaySound('cookieclicker/snd/press.mp3');}
							//PlaySound('cookieclicker/snd/tick.mp3');
						}
					}
		...
		Game.ClickSpecialPic=function()
		{
			if (Game.specialTab=='dragon' && Game.dragonLevel>=4 && Game.Has('Pet the dragon') && l('specialPic'))
			{
				...
				if (Date.now()-Game.lastClickedSpecialPic>2000) PlaySound('cookieclicker/snd/growl.mp3');
				//else if (Math.random()<0.5) PlaySound('cookieclicker/snd/growl.mp3',0.5+Math.random()*0.2);
		...
		Game.ToggleSpecialMenu=function(on)
		{
			if (on)
			{
				...
				str+='<div class="close" onclick="PlaySound(\'cookieclicker/cookieclicker/snd/press.mp3');Game.ToggleSpecialMenu(0);">x</div>';
				...
				else if (Game.specialTab=='dragon')
				{
					...
					if (Game.dragonLevel>=5)
					{
						...
						str+='<div class="crate enabled" style="opacity:1;position:absolute;right:18px;top:-58px;'+(icon[2]?'background-image:url('+icon[2]+');':'')+'background-position:'+(-icon[0]*48)+'px '+(-icon[1]*48)+'px;" '+Game.clickStr+'="PlaySound(\'cookieclicker/snd/tick.mp3\');Game.SelectDragonAura(0);" '+Game.getTooltip(
					...
					if (Game.dragonLevel>=25)//2nd aura slot; increased with last building (idleverse)
					{
						...
						str+='<div class="crate enabled" style="opacity:1;position:absolute;right:80px;top:-58px;'+(icon[2]?'background-image:url('+icon[2]+');':'')+'background-position:'+(-icon[0]*48)+'px '+(-icon[1]*48)+'px;" '+Game.clickStr+'="PlaySound(\'cookieclicker/snd/tick.mp3\');Game.SelectDragonAura(1);" '+Game.getTooltip(
